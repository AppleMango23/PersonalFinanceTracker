...
struct DashboardView: View {
    ...
    private func getCustomMonthRange(for date: Date) -> (start: Date, end: Date)? {
        ...
        // Determine the correct month for the start date
        var monthOffset = 0
        if currentDay < monthStartDate {
            monthOffset = -1 // We are in the period that started last month
        }
        
        guard let effectiveDate = calendar.date(byAdding: .month, value: monthOffset, to: date) else { return nil }
        
        var startComps = calendar.dateComponents([.year, .month], from: effectiveDate)
        startComps.day = monthStartDate
    ...
    // Only transactions in current custom month
    private var currentMonthTransactions: [Transaction] {
        guard let range = getCustomMonthRange(for: Date()) else { return [] }
...
struct CategoriesView: View {
        ...
        private func calculateTotalSpentThisMonth(for category: Category) -> Double {
        ...
        private func transactionCountThisMonth(for category: Category) -> Int {
            return currentMonthTransactions.filter { $0.category == category }.count
        }
